	...
	public void run(String paramLine) {
		...
		for (int i = 0; i < comms.length; i++)
			if (comms[i].equalsIgnoreCase(params[0]))
				ncom = i;
		if (ncom <= 0) {
			System.out.println("Pogrešna naredba: " + params[0]);
			return;
		}
					...
					try {
						...
						if (com == TABLE_UNLOCK) {
							if (Valid.getValid().runSQL(
								...
								status = "Gre�ka kod otklju�avanja!";
						}
						if (com == TABLE_UPDATE) {
							status = checkData(kdp, tables[x]);
									...
									if (!kdp.DropTable() /*|| !checkTab*/) {
										...
										if (status.endsWith("spremljen!")) {
											...
											if (status.equals("Tablica napunjena!"))
												status = "Tablica a�urirana!" + noteStatus;
			...
			public void rowTransfered(String table, int action, int row,
				...
				case raTransferNotifier.ROW_INSERT_FAILED:
					if (++errs <= 100) {
						...
						if (errs == 100)
							System.err.println("Prikazano prvih 100 gre�aka.");
					}
    ...
    public static String checkData(KreirDrop kdp, String name) {
      ...
      for (int i = 0; i < cols.length; i++) {
        ...
        if (dbc == null) {
          ...
          ret = "Gre�ka u tablici!";
          System.out.println("Gre�ka u modulu! ColumnName = "+cols[i].getColumnName()+
      ...
      if (wrongOrder) {
        ret = "Gre�ka u tablici!";
        System.out.println("pogre�an redoslijed kolona!");
        dumpColumns(cols, db);
      }
      db.close();
//      if (name.equalsIgnoreCase("zavtr")) checkbd();
	...
	public String dumpData(KreirDrop kdp) {
		try {
            ...
            if (sgt == null) return "Gre�ka kod spremanja!";
