...
public class frmMasterBlagajna extends raMasterDetail {
  ...
  raNavAction navEXIT = new raNavAction("Spremanje ra�una",raImages.IMGHISTORY,KeyEvent.VK_F10) {
    public void actionPerformed(ActionEvent e) {
      pressF10('B');
    }
  };
  ...
  raNavAction navZatvori = new raNavAction("Zatvori ra�un", raImages.IMGDOWN, KeyEvent.VK_F8) {
    public void actionPerformed(ActionEvent e) {
      closeRac();
    }
  };

  raNavAction navPonisti = new raNavAction("Arhiviraj ra�un", raImages.IMGSENDMAIL, KeyEvent.VK_F7) {
    public void actionPerformed(ActionEvent e) {
      arhRac();
    }
  };
...
//    cblag="1";
    ...
    autonext = frmParam.getParam("pos", "autonext", "D",
        "Kreirati novi pos ra�un automatski (D,N)").equals("D");
    ...
    if (presBlag.stolovi) {
      ...
      if (presBlag.stol == null || presBlag.stol.length() == 0)
        setNaslovMaster("Blagajna - svi ra�uni");
      else setNaslovMaster("Blagajna - ra�uni stola " + presBlag.stol);
      setNaslovDetail("Stavke ra�una (stol " + presBlag.stol + ")");
    
      String va = frmParam.getParam("pos", "csklVrart"+cskl, "",
          "Vrsta artikla na POS-u za skladi�te "+cskl);
      if (va.length() > 0) {
    ...
    if (ut.getFirstSecondOfDay(vl.getToday()).after(upto)) {
      if (!vl.findYear(upto).equals(god)) {
        JOptionPane.showMessageDialog(raMaster.getWindow(),
            "Pogre�na godina na predselekciji !", "Gre�ka",
            javax.swing.JOptionPane.ERROR_MESSAGE);
        return false;
      }
      int response = JOptionPane.showConfirmDialog(raMaster.getWindow(),
          "Napraviti ra�un s datumom " + Aus.formatTimestamp(upto) + " ?",
 ...
 public void this_hideDetail() {
  ...
  public void SetFokusDetail(char mode) {
    ...
    if (mode == 'N') {
      ...
      jpBl.jtfKOL.selectAll();
        ...
        javax.swing.JOptionPane.showMessageDialog(raDetail,"Nemate prava na izmjenu dokumenta !","Gre�ka",javax.swing.JOptionPane.ERROR_MESSAGE);
        raDetail.getOKpanel().jBOK_actionPerformed();    
    }
  }
  public boolean ValidacijaDetail(char mode) {
    ...
    /*if (frmParam.getParam("robno", "ugoart", "N", "Samo artikli ugostiteljstva").equalsIgnoreCase("N")) {
    	if (kolNaStanju.compareTo(getDetailSet().getBigDecimal("KOL"))<0) {
    		if (JOptionPane.showConfirmDialog(null, "Koli�ina ve�a od koli�ine na zalihi,\n�elite li nastaviti ?", "Upozorenje", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.NO_OPTION) {
    			return false;
    		}
    	}
    }*/
  ...
  public boolean DeleteCheckMaster() {
  	if (raUser.getInstance().isSuper() || presBlag.isSuper) {
        ...
        javax.swing.JOptionPane.showMessageDialog(raMaster,"Nemate prava na brisanje dokumenta !","Gre�ka",javax.swing.JOptionPane.ERROR_MESSAGE);
  		return false;
  	}
  ...
  public boolean DeleteCheckDetail() {
  	if (raUser.getInstance().isSuper() || presBlag.isSuper) {
        ...
        javax.swing.JOptionPane.showMessageDialog(raDetail,"Nemate prava na brisanje stavaka !","Gre�ka",javax.swing.JOptionPane.ERROR_MESSAGE);
  		return false;  		
  	}
...
//      getMasterSet().refresh();
      return true; // ako smo gledali stari ra�un
    }
    if (exitQuestion()) {
      return false;
    }
    ...
    System.out.println("Vra�am false");
    return false;
  }
  ...
  private void jbInit() throws Exception {
    ...
    this.setNaslovMaster("Blagajna - ra�uni");
...
//    jpBl.setDataSet(getMasterSet(), getDetailSet());
    ...
    "Definicije grupa artikala -> radno mjesto narud�be", true);
    if (np.length() > 0) {
      raDetail.addOption(new raNavAction("Ispis narud�be",
...
//    this.setJPanelDetail(this.jpDet);
    ...
    this.setNaslovDetail("Stavke ra�una");
    ...
    if (frmParam.getParam("pos", "posStolovi", "N",
      ...
      if (presBlag.isSkladOriented()) {
        ...
        this.raMaster.addOption(new raNavAction("Pregled pla�anja", raImages.IMGMOVIE, KeyEvent.VK_F7, KeyEvent.SHIFT_MASK) {
          public void actionPerformed(java.awt.event.ActionEvent ev) {
            showTotalPlac();
          }
        },5,false);
...
//    jPanel2.add(jbCKUPAC, new XYConstraints(609, 10, 21, 21));
    ...
    raMaster.getRepRunner().addReport("hr.restart.robno.repRacunPOS", "Ra�un");
    raMaster.getRepRunner().addJasper("hr.restart.robno.repRacunPOSj", "hr.restart.robno.repPosJas", "pos.jrxml", "Ra�un jasper");
    if (repFISBIH.isFISBIH()) {
      raMaster.getRepRunner().addReport("hr.restart.robno.repFISBIHPos", "Fiskalni ra�un");
      raDetail.getRepRunner().addReport("hr.restart.robno.repFISBIHPos", "Fiskalni ra�un");
    }
    raDetail.getRepRunner().addReport("hr.restart.robno.repRacunPOS", "Ra�un");
    if (np.length() > 0) {
      raDetail.getRepRunner().addReport("hr.restart.robno.repNarPOS", "Narud�ba");
    }
...
//    getDetailSet().setString("JM", jpBl.jrfJM.getText());
  ...
  void pressF7(char mode) {
    ...
    if (getDetailSet().getRowCount()>0) {
      ...
      JOptionPane.showMessageDialog(raDetail.getWindow(),
          "Nema upisanih stavaka !", "Gre�ka",JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  void arhRac() {
    ...
    if (getMasterSet().getString("STATUS").equalsIgnoreCase("P") &&
      ...
      if (JOptionPane.showConfirmDialog(raMaster.getWindow(),
          "�elite li dearhivirati ra�un?", "Dearhiviranje ra�una", 
      ...
      if (!getMasterSet().getString("STATUS").equalsIgnoreCase("N")) {
          JOptionPane.showMessageDialog(raMaster.getWindow(),
          "Ra�un je ve� prene�en!", "Arhiviranje", JOptionPane.INFORMATION_MESSAGE);
       return;
      }
      if (JOptionPane.showConfirmDialog(raMaster.getWindow(),
        "�elite li arhivirati ra�un?", "Arhiviranje ra�una", 
        JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION) return;
  ...
  void closeRac() {
    ...
    if (!getMasterSet().getString("AKTIV").equalsIgnoreCase("D")) {
      JOptionPane.showMessageDialog(raMaster.getWindow(),
          "Ra�un je ve� zaklju�en!", "Potvrda", JOptionPane.INFORMATION_MESSAGE);
      return;
    }
    
    if (JOptionPane.showConfirmDialog(raMaster.getWindow(),
        "�elite li zaklju�iti ra�un za stol "+
        getMasterSet().getString("STOL")+" ?", "Potvrda ra�una", 
        JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION) return;
...
//      raMaster.setEditEnabled(true);
  ...
  void pressF10(char mode) {
    ...
    if (getDetailSet().getRowCount()>0) {
      ...
      if (presBlag.isFiskal(getMasterSet()) && !getMasterSet().getString("FOK").equals("D")) {
        if (JOptionPane.showConfirmDialog(raDetail, "�elite li ispisati predra�un?", "Ispis", JOptionPane.OK_CANCEL_OPTION) 
      ...
      raDetail.rnvExit_action();
    	...
    	JOptionPane.showConfirmDialog(raDetail.getWindow(),"Nema upisanih stavaka !","Gre�ka",JOptionPane.DEFAULT_OPTION,JOptionPane.ERROR_MESSAGE);
      raDetail.rnvAdd_action();
    }
  }
  /**
  ...
  void split() {
    ...
    if (result != null && result.signum() > 0 && 
      ...
      try {
        ...
        JOptionPane.showMessageDialog(raDetail.getWindow(), "Gre�ka kod dodavanja stavke!",
  ...
  void transfer() {
    ...
    if (getDetailSet().rowCount() == 1) {
      JOptionPane.showMessageDialog(raDetail.getWindow(),
          "Zadnja stavka se ne mo�e prebaciti !", "Gre�ka", 
    ...
    if (dd == null) {
      JOptionPane.showMessageDialog(raDetail.getWindow(),
          "Stavka nije snimljena !", "Gre�ka", JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    if (!checkRate()) {
      JOptionPane.showMessageDialog(raDetail.getWindow(),
          "Gre�ka kod snimanja rata !", "Upozorenje", 
...
/*  boolean pressESC() {
  ...
  boolean exitQuestion() {
    ...
    if (getDetailSet().rowCount()>0)  {
      if (!newRacun || getMasterSet().getInt("BRDOK") != brdoknew) return false; // ako smo gledali stari ra�un
      if (JOptionPane.showConfirmDialog(null, "�elite li poni�titi unos ?", "Izlaz", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
  ...
  public void printNar() {
    ...
    if (!printNar_impl(true, true))
      JOptionPane.showMessageDialog(raDetail.getWindow(),
          "Pogre�no zadane grupe artikala za ispis narud�bi!", "Gre�ka",
          JOptionPane.ERROR_MESSAGE);
  }
  ...
  public boolean printNar_impl(boolean saveChanges, boolean warn) {
    String np = frmParam.getParam("pos", "narPrint", "",
        "Definicije grupa artikala -> radno mjesto narud�be");
    ...
    if (!needSave && warn) {
      if (ina == 1)
        JOptionPane.showMessageDialog(raDetail.getWindow(), 
            "Nema stavaka za ispis narud�be!", "Upozorenje",
            JOptionPane.WARNING_MESSAGE);
      else if (JOptionPane.OK_OPTION != JOptionPane.showConfirmDialog(
          raDetail.getWindow(), "Nema prethodno neispisanih stavaka za narud�bu!\n" +
            "Ispisati narud�bu iznova (oprez)?", "Narud�ba",
...
//        presBlag.isFiskGot(ms) ? presBlag.getFiskNapG(ms) : presBlag.getFiskNap(ms), // oznaka naplatnog mjesta
  ...
  */
  ...
  /* TODO: hernad ractype, fiskaliziraj
...
/*    if (getMasterSet().getBigDecimal("UIRAC").doubleValue()==0) {
      JOptionPane.showConfirmDialog(null,"Nema upisanih stavaka !","Gre�ka",JOptionPane.DEFAULT_OPTION,JOptionPane.ERROR_MESSAGE);
    ...
    raDetail.rnvExit_action();
//    pressF10('B');
//    _Main.getStartFrame().showFrame("hr.restart.robno.dlgPrintPOS", "Spremanje i ispis ra�una");
/*
 ...
 */
  ...
  public void justPrintGRC() {
    int maxGrcF5 = Integer.parseInt(hr.restart.sisfun.frmParam.getParam("pos", "GRC_kom_int", "1", "Broj ra�una kod ispisa na F5 iz unosa POS-a",true));
    String oneTimeReportName = "hr.restart.robno.repRacunPOS";
    if (repFISBIH.isFISBIH() && frmParam.getParam("robno", "FISBIHdirPos", "N", "Koristiti BiH fiskalni ra�un za direktni ispis sa POS-a", true).equalsIgnoreCase("D")) {
...
//    });
  ...
  boolean checkKol() {
    ...
    if (kolAvail.add(oldk).compareTo(getDetailSet().getBigDecimal("KOL")) < 0)
      if (JOptionPane.showConfirmDialog(raDetail.getWindow(), 
          "Koli�ina ve�a od koli�ine artikla na zalihi,\n" +
          "�elite li nastaviti ?", "Upozorenje", JOptionPane.OK_CANCEL_OPTION, 
  ...
  void showRequirementsMaster() {
    ...
    viewReq.setTitle("Prikaz arhivirane potro�nje materijala  za odabrano razdoblje");
