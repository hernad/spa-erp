...
public class presBlag extends PreSelect {
  ...
  public void SetFokus() {
     ...
     * definiranog maloprodajnog skladi�ta !",
  ...
  public boolean Validacija() {
    ...
    if (isUserOriented()) {
      ...
      if (!lookupData.getlookupData().raLocate(dm.getBlagajnici(), "LOZINKA",
        ...
        JOptionPane.showMessageDialog(null, "Pogre�na lozinka!", "Gre�ka",
    ...
    if (frmMasterBlagajna.allReadyRun == true) {
      JOptionPane.showConfirmDialog(null, "Blagajna ve� pokrenuta !", "Gre�ka",
  ...
  void jbInit() throws Exception {
    ...
    jLabel1.setText(isSkladOriented() ? "Skladi�te" : "Prodajno mjesto");
  ...
  public static boolean isSkladOriented() {
    if (skladOriented < 0) {
      skladOriented = frmParam.getParam("pos", "skladPos", "N",
          "Da li se POS vodi na nivou skladi�ta (D) ili prodajnog mjesta")
  ...
  */
  ...
  public static int getFiskNap(DataSet ms) {
    ...
    if (ms.getString("VRDOK").equals("IZD")) {
      String io = frmParam.getParam("robno", "fiskNapI-"+myorg.getString("FPP"), "", 
          "Oznaka naplatnog ure�aja za izdatnice PP " + myorg.getString("FPP") + " (lokalno)", true);
      if (io != null && io.length() > 0) Aus.getNumber(io);
    }
    String ur = frmParam.getParam("robno", "fiskNap-"+myorg.getString("FPP"), "1", 
        "Oznaka naplatnog ure�aja PP " + myorg.getString("FPP") + " (lokalno)", true);
  ...
  public static int getFiskNapG(DataSet ms) {
    ...
    String ur = frmParam.getParam("robno", "fiskNapG-"+myorg.getString("FPP"), "2", 
        "Oznaka naplatnog ure�aja za gotovinu PP " + myorg.getString("FPP") + " (lokalno)", true);
    if (ur == null || ur.length() == 0) return 1;
    return Aus.getNumber(ur);
  }
  ...
  public static boolean isFiskSep() {
    if (fisksep < 0) {
      fisksep = frmParam.getParam("robno", "fiskPojed", "N",
          "Broja� fiskalizacije po naplatnom mjestu (D,N)")
          .equalsIgnoreCase("D") ? 1 : 0;
    }
    return fisksep == 1;
  }
  ...
  public static int getFiskNapG() {
    String ur = frmParam.getParam("robno", "fiskNapG", "2", "Oznaka naplatnog ure�aja za gotovinu (lokalno)", true);
    if (ur == null || ur.length() == 0) return 1;
    return Aus.getNumber(ur);
  }
  ...
  public static int getFiskNap(String cskl) {
    if (isSkladOriented()) {
      String ur = frmParam.getParam("robno", "fiskNap"+cskl, "1", "Oznaka naplatnog ure�aja " + cskl + " (lokalno)", true);
      if (ur == null || ur.length() == 0) return 1;
      return Aus.getNumber(ur);
    }
    return getFiskNap();
  ...
  public static int getFiskNap() {
    ...
    String ur = frmParam.getParam("robno", "fiskNap", "1", "Oznaka naplatnog ure�aja (lokalno)", true);
    if (ur == null || ur.length() == 0) return 1;
    return Aus.getNumber(ur);
  }*/
