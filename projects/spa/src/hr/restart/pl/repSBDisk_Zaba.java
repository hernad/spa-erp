...
public class repSBDisk_Zaba extends repDisk{
  StorageDataSet qds;
...
//    String brRac = vl.maskString(zabaDS.getString("BROJTEK")+"000000",'0',16);
    String brRac = vl.maskString(zabaDS.getString("BROJTEK")+"0000000",'0',17);
    String mBr = getMatBr()+"000";
    String ibPB = getIBPB()+"*   ";

    sb.replace(0, 0+"03100000000000000".length(),"03100000000000000");
    sb.replace(16, 16+date.length(),date);
    sb.replace(22, 22+"0150".length(),"0150");
    sb.replace(26, 26+iznos.length(),iznos);
    sb.replace(37, 37+brRac.length(),brRac);
    sb.replace(53, 53+mBr.length(),mBr);
...
//  }
  ...
  {
    ld.raLocate(dm.getIsplMJ(), new String[]{"CISPLMJ"}, new String[]{fBS.getFieldSet().getString("CISPLMJ")});
//    ld.raLocate(dm.getIsplMJ(), new String[]{"TIPFILE"}, new String[]{tipFile});
System.out.println("cbanke is "+dm.getIsplMJ().getInt("CBANKE"));    
...
//      {
//        System.out.println("PATH: " + roots[i].getPath()) ;
//      }
//  }
