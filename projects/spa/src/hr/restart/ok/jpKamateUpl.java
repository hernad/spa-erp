...
public class jpKamateUpl extends JPanel {
  raCommonClass rcc = raCommonClass.getraCommonClass();
  dM dm = dM.getDataModule();
  Valid vl = Valid.getValid();

  frmKamateUpl fKamateUpl;
  ...
  JraButton jbSelBr = new JraButton();
  JlrNavField jlrBrojdok = new JlrNavField();
  JlrNavField jlrCracuna = new JlrNavField();
  JraTextField jraDatdok = new JraTextField();
  JraTextField jraIznos = new JraTextField();
  JlrNavField jlrCpar = new JlrNavField() {
    public void after_lookUp() {
      fKamateUpl.partnerChanged();
    }
  };
  ...
  public jpKamateUpl(frmKamateUpl f) {
    try {
      fKamateUpl = f;
      jbInit();
    }
  ...
  public void BindComponents(DataSet ds) {
    jlrBrojdok.setDataSet(ds);
    jlrCracuna.setDataSet(ds);
  ...
  private void jbInit() throws Exception {
    ...
    jlrBrojdok.setColumnName("BROJDOK");
    jlrBrojdok.setColNames(new String[] {"CRACUNA"});
    jlrBrojdok.setTextFields(new JTextComponent[] {jlrCracuna});
    jlrBrojdok.setVisCols(new int[] {2, 5, 6});
    jlrBrojdok.setSearchMode(0);
    jlrBrojdok.setRaDataSet(dm.getKamRac());
    jlrBrojdok.setNavButton(jbSelBr);

    jlrCracuna.setColumnName("CRACUNA");
    jlrCracuna.setNavProperties(jlrBrojdok);
    jlrCracuna.setEnabled(false);
    jlrCracuna.setVisible(false);
    ...
    jpDetail.add(jlIznos, new XYConstraints(315, 70, -1, -1));
    jpDetail.add(jlrCpar, new XYConstraints(150, 20, 100, -1));
    jpDetail.add(jlrNazpar, new XYConstraints(255, 20, 250, -1));
    jpDetail.add(jlrBrojdok, new XYConstraints(150, 45, 355, -1));
    jpDetail.add(jbSelBr, new XYConstraints(510, 45, 21, 21));
    jpDetail.add(jlrCracuna, new XYConstraints(140, 70, 5, -1));
    jpDetail.add(jraDatdok, new XYConstraints(150, 70, 100, -1));
    jpDetail.add(jraIznos, new XYConstraints(405, 70, 100, -1));
