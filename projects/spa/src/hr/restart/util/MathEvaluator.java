...
public class MathEvaluator {
  ...
  private HashMap variables = new HashMap();
    ...
    public static void main(String[] args)  {
      if ( args == null || args.length != 1) {
        System.err.println("Math Expression Evaluator stealed by AI C(2001):");
  ...
  */
  ...
  private void initializeOperators() {
    ...
    protected class Node  {
      ...
      private void init(Node parent, String s, int level) throws Exception {
          ...
          if ( nOperator != null ) {
            ...
            if ( startOperator==0 && nOperator.getType() == 1 ) {
              ...
              if ( checkBrackets( s.substring( nOperator.getOperator().length() ) ) == 0 ) {
                ...
                nRight = null;
                return;
              } else throw new Exception("Error during parsing... missing brackets in [" + s + "]");
            } else if ( startOperator > 0 && nOperator.getType() == 2 ) {// two operands
              nOperator = nOperator;
            ...
            }// startOperator==0 && nOperator.getType() == 1
          }//nOperator != null
        }
        ...
        protected int checkBrackets(String s) {
          ...
          int inBracket = 0;
          for (int i=0; i<sLength; i++) {
            if ( s.charAt(i) == '(' && inBracket >= 0 ) inBracket++;
    ...
    /**
     * Dovlaci vrijednost racunske operacije izmedju BigDecimal kolona u zadanom readrowu. 
     * @param statement npr. ID - IP / 100
     * @param set readrow sa kolonama koje se spominju u statementu (npr. ID, IP)
     * @return rezultat statementa nakon sto su nazivi BigDecimal kolona zamjenjeni njihovim vrijednostima  
     */
    public static BigDecimal getEvaluatedBigDecimal(String statement, ReadRow set) {
      BigDecimal bd = new BigDecimal(0);
      if (statement == null 
          || statement.equals("") 
          || statement.equalsIgnoreCase("null")) return bd;
      MathEvaluator m = new MathEvaluator(statement);
      ...
      for (int i = 0; i < cn.length; i++) {
        if (cn[i].getDataType() == com.borland.dx.dataset.Variant.BIGDECIMAL) {
          try {
            ...
            System.out.println("MathEvaluator.getEvaluatedBigDecimal#addVariable ("+statement+") :: "+e);
          }
        }
      }
      try {
        ...
        System.out.println("MathEvaluator.getEvaluatedBigDecimal#getValue ("+statement+") :: "+e);
      }
      return bd;
    }
